...
class Effects:
    ...
    async def execute(self, operation, arguments, **kw):
        self.dispatched.append((operation, arguments))
        await kw["progress"](0.25, "accepted")
        return {"reply": 7}
    ...
    async def observe_effect(self, operation, arguments, accepted, **kw):
        while not self.release.is_set():
            if kw["cancel"].is_set():
                ...
                return {"evidence": [{"cancelled": True}], "result": {}}
            await asyncio.sleep(0)
        return {
            "evidence": [{"kind": "fixture", "operation": operation}],
...
def setup(capabilities=None, compat=None):
    ...
    instance = Instance(
        ...
        1,
        TOKEN,
        {"class": "C64", "model": "PAL", "emulator": "x64sc", "version": "3.10"},
        "stopped",
        "paused",
        capabilities or set(OPS),
        transport,
    )
    ...
    return {
        ...
        "params": {
            ...
            "arguments": {
                ...
                "target": {"instance_id": INSTANCE, "generation": 1, "lease_token": TOKEN},
                "deadline_ms": 1000,
                **(arguments or {}),
            },
        },
...
async def test_missing_capability_rejected_before_transport_dispatch():
    server, transport = setup({"vice.ping"})
    await initialize(server)
    response = await server.handle(call())
    assert payload(response)["error"]["code"] == "UNSUPPORTED_COMMAND"
...
async def test_success_waits_for_effect_evidence_and_emits_progress():
    ...
    notes = []
    task = asyncio.create_task(server.handle(call(), notes.append))
    await asyncio.sleep(0)
...
async def test_cancellation_notification_is_structured_error():
    ...
    await initialize(server)
    transport.release.clear()
    task = asyncio.create_task(server.handle(call(request_id=44)))
    await asyncio.sleep(0)
    await server.handle(
        {"jsonrpc": "2.0", "method": "notifications/cancelled", "params": {"requestId": 44}}
    )
    response = await task
    assert payload(response)["error"]["code"] == "CANCELLED"
    assert payload(response)["error"]["effect_may_have_occurred"]
...
async def test_explicit_legacy_mode_returns_audited_bare_shape():
    server, _ = setup(compat="legacy-bare-result-v1")
    await initialize(server)
...
async def test_core_operations_propagate_verified_effect(name, args):
    server, transport = setup()
    await initialize(server)
    value = payload(await server.handle(call(name, args)))
...
async def test_jsonrpc_errors_are_separate_from_domain_errors():
    server, _ = setup()
    assert (await server.handle({"jsonrpc": "2.0", "id": 1, "method": "tools/list"}))["error"][
        "code"
    ] == -32002
    await initialize(server)
    assert (await server.handle({"jsonrpc": "2.0", "id": 2, "method": "nope"}))["error"][
        "code"
    ] == -32601
