...
def _run(root: Path, workers: int):
    ...
    def execute(case, artifact, port):
        i = case["variant"]
        marker = f"{case['machine']}:{i}:{port}"
        (artifact / "instance.json").write_text(
            json.dumps({"variant": i, "marker": marker}), encoding="utf-8"
        )
        # Distinct per-instance memory/drive sentinels; any cross-talk changes this.
        time.sleep(0.002)
        value = (0xA50000 + i) ^ (port << 1)
        (artifact / "sentinel.json").write_text(
            json.dumps({"ram": value, "drive": value ^ 0x55AA}), encoding="utf-8"
        )
        return {
...
def test_w3_a2_serial_parallel_byte_equivalent(tmp_path):
    serial = _run(tmp_path / "serial", 1)
    parallel = _run(tmp_path / "parallel", 4)

    def norm(p):
        return [
            (r["case_id"], r["status"], r["provenance"]["instance_id"], r["provenance"]["sentinel"])
            for r in p["results"]
        ]
...
def test_w3_a2_two_supervisors_same_base_are_disjoint():
    ...
    b = threading.Thread(target=launch)
    a.start()
    b.start()
    a.join()
    b.join()
