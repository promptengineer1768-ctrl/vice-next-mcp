...
def test_runner_outputs_and_artifacts(tmp_path):
    def execute(case, artifact, port):
        (artifact / "sample.txt").write_text(str(port)); return {"executable_hash": "x", "raw_samples": [1]}
    result = BatchRunner(tmp_path, workers=2).run([{"machine":"c64","variant":i} for i in range(4)], execute)
