...
def _cases(path: Path) -> list[dict]:
    ...
    if isinstance(value, dict): value=value.get("cases", [value])
    if not isinstance(value, list) or not all(isinstance(x, dict) for x in value):
        raise ValueError("cases JSON must be an array of objects (or {cases:[...]})")
    return value

def batch_main(ns: argparse.Namespace) -> int:
    cases=_cases(Path(ns.cases)); stop=threading.Event()
    def interrupt(*_): stop.set()
    signal.signal(signal.SIGINT, interrupt)
    def execute(case, artifact, port):
        artifact.joinpath("case.json").write_text(json.dumps(case,indent=2,sort_keys=True),encoding="utf-8")
        command=case.get("command") or ns.command
        if not command: return {"mode":"managed", "port":port}
        if isinstance(command,str): shell=True; argv=command
        else: shell=False; argv=[str(x) for x in command]
        env=os.environ.copy(); env.update({"VICE_MCP_PORT":str(port),"VICE_MCP_ARTIFACT_DIR":str(artifact)})
        proc=subprocess.Popen(argv,cwd=case.get("cwd"),env=env,shell=shell,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
        try:
            stdout,stderr=proc.communicate(timeout=ns.timeout)
        except subprocess.TimeoutExpired:
            proc.kill(); stdout,stderr=proc.communicate(); raise RuntimeError("command timed out")
        if stop.is_set() and proc.poll() is None: proc.terminate()
        ...
        if proc.returncode: raise RuntimeError(f"command exited {proc.returncode}")
        return {"mode":"command", "returncode":proc.returncode, "port":port}
    result=BatchRunner(ns.artifact_root,workers=ns.workers,base_port=ns.base_port).run(cases,execute,fail_fast=ns.fail_fast,cancel=stop)
    out=json.dumps(result,indent=2,sort_keys=True); print(out)
...
def main(argv=None):
    ...
    b.add_argument("--base-port",type=int,default=int(os.getenv("VICE_MCP_BASE_PORT","0")) or None)
    ...
    m.add_argument("--machine", choices=("x64sc", "x128"), default="x64sc")
    ...
    def stdio(ns):
        async def serve():
            ...
            print(json.dumps({"event": "instance_ready", "instance_id": instance.id,
                              "generation": instance.generation, "lease_token": lease.token,
                              "machine": ns.machine}), flush=True)
            try:
        ...
        asyncio.run(serve()); return 0
    m.set_defaults(func=stdio)
    ns=p.parse_args(argv); return ns.func(ns)
