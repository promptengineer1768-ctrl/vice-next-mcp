...
def vice_artifact_dir(request, tmp_path_factory):
    worker=os.environ.get("PYTEST_XDIST_WORKER","gw0"); run=uuid.uuid4().hex[:10]
    node=sanitize(request.node.nodeid)
    path=tmp_path_factory.mktemp("vice")/f"{run}-{worker}-{node}"
    path.mkdir(parents=True,exist_ok=True); return Path(path)
...
def vice_instance_factory(vice_artifact_dir):
    ...
    def factory(machine="c64", **metadata):
        item={"machine":machine,"generation":1,"artifact_dir":str(vice_artifact_dir),**metadata}; instances.append(item); return item
    yield factory
