...
class BinaryMonitorTransport:
    ...
    def execute(self,instance_id,operation,**params):
        ...
        if operation=='memory.read': result=m.memory(int(params['address']),int(params['length']),int(memspace),int(params.get('bank',0)))
        elif operation=='memory.write': result=m.memory_write(int(params['address']),bytes(params['data']),int(memspace),int(params.get('bank',0)))
            ...
            raise RuntimeError('instrumented VICE event bridge is not connected to this monitor session')
    ...
    def keyboard_type(self, instance_id: str, text: str):
        ...
        return {'result': {'count': len(data), 'mode': 'vice-binary-keyboard-feed'},
                'evidence': OperationEvidence('keyboard.type', i.id, i.generation,

                    i.state, {'monitor_command': '0x72', 'api_version': 2,
                              'physical_matrix': False, 'compatibility_shadow': True,
                              'count': len(data)}).__dict__}

    def keyboard_matrix(self, instance_id: str, *, row: int, column: int,
                        action: str = 'press', frames: int = 1):
        ...
        if 'vice.keyboard.matrix' not in i.capabilities:
            raise RuntimeError('native VICE matrix injection is unavailable; instrumented VICE is required')
        pressed = action in {'press', 'hold'}
        for _ in range(int(frames)):
            i.monitor.keyboard_matrix(row, column, pressed)
        return {'result': {'row': int(row), 'column': int(column), 'action': action, 'frames': int(frames)},
                'evidence': OperationEvidence('keyboard.matrix', i.id, i.generation, i.state,
                    {'monitor_command': '0x74', 'physical_matrix': True}).__dict__}
    ...
    def keyboard_restore(self, instance_id: str, *, action: str):
        ...
        if 'vice.keyboard.restore' not in i.capabilities:
            raise RuntimeError('native VICE RESTORE injection is unavailable; instrumented VICE is required')
        i.monitor.keyboard_matrix(-3, 0, action == 'press')
        return {'result': {'action': action}, 'evidence': OperationEvidence('keyboard.restore', i.id,
            i.generation, i.state, {'monitor_command': '0x74', 'physical_restore': True}).__dict__}
