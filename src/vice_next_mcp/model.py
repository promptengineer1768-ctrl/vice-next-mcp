...
class StaticResolver:
    def __init__(self, instances):
        self.instances = {x.instance_id: x for x in instances}
    ...
    async def resolve(self, target):
        ...
        if target.get("generation") != value.generation:
            raise ViceError(
                "STALE_GENERATION", "instance generation is stale", {"expected": value.generation}
            )
        if target.get("lease_token") != value.lease_token:
            raise ViceError("LEASE_REQUIRED", "a valid instance lease is required")
        return value
