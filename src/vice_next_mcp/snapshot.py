...
def validate_snapshot(path: str | Path, *, required_modules: Iterable[str] = ()) -> dict[str, Any]:
    p = Path(path)
    ...
    while pos + 16 <= len(data):
        ...
        if not name or length < 0 or pos + 16 + length > len(data):
            break
        modules.append(name)
        pos += 16 + length
    missing = [m for m in required_modules if m not in modules]
    if missing:
        raise SnapshotError(f"snapshot missing required modules: {missing}")
...
def ensure_path(path: str | Path, root: str | Path | None = None) -> Path:
    p = Path(path).resolve()
