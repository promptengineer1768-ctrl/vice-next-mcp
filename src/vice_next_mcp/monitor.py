...
class CommandError(TransportError):
    def __init__(self, r: Response):
        ...
        super().__init__(
            f"VICE command ${r.command:02x} request {r.request_id} failed: ${r.error:02x}"
        )
...
async def read_response(reader: asyncio.StreamReader) -> Response:
    try:
        ...
        raise Disconnected("VICE closed monitor stream") from e
    if header[:2] != MAGIC:
        raise ProtocolError(f"invalid monitor magic {header[:2].hex()}")
    size = struct.unpack_from("<I", header, 2)[0]
    if size > MAX_BODY:
        raise ProtocolError(f"invalid response length {size}")
    try:
        body = await reader.readexactly(size)
    except asyncio.IncompleteReadError as e:
        raise Disconnected("VICE closed during response body") from e
...
class BinaryMonitorTransport:
    ...
    def __init__(
        ...
        *,
        ...
        if max_pending < 1 or max_events < 1:
        ...
        self._events = asyncio.Queue(max_events)
    ...
    async def connect(self):
        async with self._life:
            ...
            try:
                ...
                self.failure = e
                raise Disconnected(f"cannot connect to {self.host}:{self.port}") from e
            self.state = State.CONNECTED
            self.failure = None
            self._task = asyncio.create_task(self._read_loop(), name=f"vice-monitor-{self.port}")

    def _request_id(self):
        for _ in range(EVENT_ID):
            self._next = (self._next + 1) & 0xFFFFFFFF
            if self._next == EVENT_ID:
                self._next = 0
    ...
    async def negotiate(self):
        return await self.execute(0x85)
    ...
    async def execute(self, command: int, body: bytes = b"", *, timeout=None):
        ...
        future = None
        try:
            if self.state is not State.CONNECTED or self._writer is None:
                raise Disconnected(f"monitor is {self.state.value}")
            rid = self._request_id()
            future = asyncio.get_running_loop().create_future()
            self._pending[rid] = future
            async with self._write:
                self._writer.write(encode_request(command, rid, body))
            ...
            try:
                ...
                raise TimeoutError(
                    f"monitor request {rid} timed out; effect may have occurred"
                ) from e
            if reply.error:
    ...
    async def next_event(self, *, timeout=None):
        return (
            ...
            if timeout is None
            else await asyncio.wait_for(self._events.get(), timeout)
        )
        ...
        try:
            while True:
                ...
                if reply.request_id == EVENT_ID:
                    ...
                    self._events.put_nowait(reply)
                    continue
                future = self._pending.get(reply.request_id)
                if future is None:
                    raise ProtocolError(f"unexpected request id {reply.request_id}")
                if not future.done():
            ...
            for future in tuple(self._pending.values()):
                if not future.done():
                    future.set_exception(Disconnected(str(e)))
            if self._writer:
                self._writer.close()
    ...
    async def close(self):
        async with self._life:
            ...
            for future in tuple(self._pending.values()):
                if not future.done():
                    future.set_exception(Disconnected("monitor closed"))
            self._pending.clear()
