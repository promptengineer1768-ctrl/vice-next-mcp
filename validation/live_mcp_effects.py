...
async def run(exe: str, out: Path):
    ...
    try:
        r = await runtime.handle(
            {
                "jsonrpc": "2.0",
                "id": 2,
                "method": "tools/call",
                "params": {"name": "vice.memory.read", "arguments": args},
            }
        )
        rows.append(
            {
                "operation": "vice.memory.read",
                "status": "passed" if not r["result"]["isError"] else "failed",
                "response": r,
            }
        )
                ...
                rows.append(
                    {
                        "operation": op,
                        "status": "passed",
                        "response": t.execute(i.id, op, **kw),
                        "exists": p.exists(),
                    }
                )
            except Exception as e:
                rows.append({"operation": op, "status": "failed", "error": repr(e)})
        rows += [
            {"operation": x, "status": "unsupported", "reason": "runtime capability not exposed"}
            for x in ("keyboard", "trace", "iec_capture")
        ]
    finally:
        lease.release()
        s.close()
    payload = {
        "schema": "vice-next-mcp/live-mcp-effects/1",
        "evidence_class": "live",
        "executable": exe,
        "rows": rows,
    }
    out.mkdir(parents=True, exist_ok=True)
    (out / "live-mcp-effects.json").write_text(
...
def main():
    ...
    a = ap.parse_args()
    asyncio.run(run(a.executable, Path(a.output)))
