...
def configurations() -> list[tuple[str, float, dict[int, float]]]:
    hosts = [
        ("c64-pal", PAL_C64_HZ),
        ("c64-ntsc", NTSC_C64_HZ),
        ("c128-1mhz", PAL_C64_HZ),
        ("c128-2mhz", DRIVE_2MHZ_HZ),
    ]
    topologies = [
        ("1541", {8: DRIVE_1541_HZ}),
        ("1571", {8: DRIVE_2MHZ_HZ}),
        ("1581", {8: DRIVE_2MHZ_HZ}),
        ("mixed-2", {8: DRIVE_1541_HZ, 9: DRIVE_2MHZ_HZ}),
...
def run_case(
    case_id: str, host_hz: float, clocks: dict[int, float], blocks: int, faults: int, seed: int
) -> CaseResult:
    rng = random.Random(seed)
    simulator = IecClusterSimulator(
        {d: 1 for d in clocks},
        deadline=max(100_000, (blocks + faults + 1) * 100_000),
        host_clock=ClockDomain(host_hz),
    ...
    for i in range(blocks):
        ...
        try:
            result = simulator.adaptive_transaction(
                device,
                1,
                payload,
                reply=DriveReply(0, payload),
                cable=CableModel(1.0),
                transient_noise_attempts=set(),
            )
            ...
            checked += sum(
                1
                for e in simulator.bus.trace
                if e.action.startswith("device") or "complete" in e.action
            )
    ...
    return CaseResult(
        ...
        failures,
        {
            "trace_events": len(simulator.bus.trace),
            "bus_idle": idle,
            "seed": seed,
            "clock_hz": host_hz,
        },
...
def main() -> int:
    ...
    args = ap.parse_args()
    results = [
        run_case(cid, hz, drives, args.blocks, args.faults, args.seed + n)
        for n, (cid, hz, drives) in enumerate(configurations())
    ]
    report = {
        "schema": "w6d-conformance-1",
        "evidence": "modelled",
        "measured_cases": 0,
    ...
    report["reproducibility"] = {
        "command": "python validation/conformance_matrix.py --blocks 10000 --faults 100 --seed %d"
        % args.seed,
